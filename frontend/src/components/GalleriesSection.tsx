...
import { fetchPublicGalleries, UserSet } from "../api/userSetApi";
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {galleries.map((gallery) => {
          ...
          return (
            <Link
              key={gallery.id}
              to={`/gallery/${encodeURIComponent(gallery.id)}`}
            ...
            >
              <div className="h-40 overflow-hidden">
                {thumbnailUrl ? (
                  <img
                    src={thumbnailUrl}
                    alt={typeof gallery.title === "object" ? stripLastWord(gallery.title) : gallery.title}
              ...
              <div className="p-4">
                <h3 className="text-xl font-semibold">
                  {typeof gallery.title === "object" ? stripLastWord(gallery.title) : gallery.title}
                </h3>
                {gallery.description && gallery.description.en && (
                  <p className="text-sm text-gray-600 mt-2">{gallery.description.en[0]}</p>
                )}
              </div>
            </Link>
          );
