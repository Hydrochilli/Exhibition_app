...
const FiltersSection: React.FC<{
  ...
  selectedApi: string; // "All", "The Met", "Cleveland"
    ...
    <div className="flex flex-col md:flex-row gap-4 justify-center my-4">
      ...
      <div>
        ...
        >
          ...
          {/* Add more if you want, but be sure to define them in the "map" in unifySearch */}
        </select>
      </div>
...
const Home: React.FC = () => {
  ...
  // We store a "query" that triggers <SearchResults> once "Apply Filters" is pressed
  ...
  // Called from FiltersSection "Apply" button
  const applyFilters = () => {
    // If you want to allow an empty search, handle that here
    // We'll simply store whatever the user typed
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {query ? (
        // Once user clicks "Apply", show <SearchResults> with filters
        <SearchResults
          searchTerm={query}
          century={century}
          department={department}
        ...
        // If user typed a search but didn't press "Apply", show unfiltered
        <SearchResults searchTerm={searchTerm} />
      ) : (
        // No search => show curated home
        <>
          <CuratedCategories />
          <GalleriesSection />
        </>
