...
export const fetchEuropeanaArtworks = async (searchTerm, page, pageSize, typeFilter, centuryFilter) => {
    try {
...
const normalizeCollectionId = (rawId) => {
    let id = rawId;
    if (id.includes("http")) {
...
export const fetchRepresentativeImageForCollection = async (rawCollectionId, collectionTitle) => {
...
export const fetchEuropeanaCollections = async (queryParam) => {
    try {
        ...
        const collectionsData = response.data.items.map(async (collection) => {
            const imageUrl = await fetchRepresentativeImageForCollection(collection.id, collection.title?.[0]);
            let description = "No description available";
            if (collection.dcDescriptionLangAware &&
...
export const fetchEuropeanaCollectionArtworks = async (collectionId, page = 1, pageSize = 48) => {
    try {
        const response = await axios.get(EUROPEANA_BASE_URL, {
            params: {
                wskey: EUROPEANA_API_KEY,
